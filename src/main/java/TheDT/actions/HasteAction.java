...
public class HasteAction extends AbstractGameAction {
	private static final UIStrings uiStrings = CardCrawlGame.languagePack.getUIString(DTModMain.makeID("HasteAction"));
	...
	public void update() {
		if (this.duration == Settings.ACTION_DUR_FAST) {
			...
			if (upgraded || p.hand.group.size() - nonPowers.size() == 1) {
				for (AbstractCard c : p.hand.group) {
					if (c.type == AbstractCard.CardType.POWER) {
