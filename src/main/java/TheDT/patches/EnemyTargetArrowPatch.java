	...
	public static class MonsterGroupRenderPatch {
		...
		public static void Postfix(MonsterGroup __instance, SpriteBatch sb) {
			if (AbstractDungeon.player instanceof DragonTamer) {
				if (ChooseAttackerAction.activeThis != null || AbstractDungeon.player.hoveredCard != null) {
					arrowTime = 0.0f;
					alpha = 0.0f;
					hoveredMonster = null;
				...
				if (DTModMain.enemyTargetDisplayConfig[2]) {
				...
				if (DTModMain.enemyTargetDisplayConfig[3]) {
					if (DTModMain.targetMarker.hb.hovered) {
						hoveredMonster = null;
